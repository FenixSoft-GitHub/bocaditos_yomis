...
function useTooltipContext(componentName: string): TooltipContextValue {
  ...
  if (!context) {
    throw new Error(
      `Tooltip must be used within a Tooltip Context. Component: ${componentName}`
    );
  }
  return context;
}
    ...
    <g
      ref={(node) => {
        ...
        if (typeof forwardedRef === "function") {
          ...
          (forwardedRef as React.MutableRefObject<SVGGElement | null>).current =
            node;
        }
      }}
      onPointerMove={(event) => {
...
>((props, ref) => {
  ...
  // Calcula la posición basada en el viewport
  ...
  return createPortal(
    isMobile ? (
      <div
        ref={ref} // ✅ Puedes pasar `ref` aquí si lo necesitas
